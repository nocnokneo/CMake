                                            ...
                                            const std::string& language) const
{
  ...
  if(this->Makefile->IsOn("CMAKE_DISABLE_PRECOMPILED_HEADERS"))
    {
    return std::string();
    }
  ...
  if(inserted.second)
    ...
    this->GetPrecompileHeaders(headers, config, language);
    if(headers.empty())
      {
      return std::string();
      }
    std::string &filename = inserted.first->second;
    filename = this->LocalGenerator->GetCurrentBinaryDirectory();
    filename += "/PCH/";
    filename += this->GetName();
    filename += language;
    filename += config;
    filename += ".h";
    cmGeneratedFileStream file(filename.c_str());
                                            ...
                                            const std::string& language) const
{
  ...
  if(inserted.second)
    {
    std::string pchHeader = this->GetPchHeader(config, language);
    if(pchHeader.empty())
      {
      return std::string();
      }
